...
- Configuration values should be read in `config/runtime.exs` using `System.get_env/1` and raise an error if required values are missing
- **Never** use `Application.compile_env/3` with default values - all configuration must come from the environment
- Use `Application.get_env/2` for runtime configuration access (loaded from `config/runtime.exs`)
