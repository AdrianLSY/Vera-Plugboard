defmodule Plugboard.Services.Services do
  ...
  def delete_service(%Service{} = service) do
    ...
    case result do
      {:ok, {updated_service, descendants}} ->
        notify_subscribers({:ok, updated_service}, [
          :service,
          :deleted,
          descendants,
          service.parent_id
  ...
  def create_service_api_token(service) do
    ...
    token = %{
      ...
      expires_at:
        DateTime.add(record.inserted_at, @service_token_validity_in_days * 24 * 60 * 60, :second)
    }
  ...
  defp notify_subscribers({:ok, service}, [:service, :updated]) do
    ...
    PubSub.broadcast(
      Plugboard.PubSub,
      "service/#{service.id}",
      {:path_updated, Service.full_path(service)}
    )
    ...
    |> Enum.each(fn descendant ->
      PubSub.broadcast(
        Plugboard.PubSub,
        "service/#{descendant.id}",
        {:path_updated, Service.full_path(descendant)}
      )
    end)
  ...
  defp notify_subscribers({:ok, service}, [:service, :deleted, descendants, redirect_service_id]) do
    PubSub.broadcast(
      Plugboard.PubSub,
      "service/#{service.id}",
      {:service_deleted, service, redirect_service_id}
    )
    ...
    |> Enum.each(fn descendant ->
      PubSub.broadcast(
        Plugboard.PubSub,
        "service/#{descendant.id}",
        {:service_deleted, descendant, redirect_service_id}
      )
    end)
